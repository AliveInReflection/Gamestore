    ...
    public class GameService : IGameService
    {
        ...
        public void Create(GameDTO game, IEnumerable<int> genreIds, IEnumerable<int> platformTypeIds)
        {
            ...
            catch (InvalidOperationException)
            {
                ...
                {
                    var genreEntry = database.Genres.Get(m => m.GenreId.Equals(id));
                ...
                {
                    var ptEntry = database.PlatformTypes.Get(m => m.PlatformTypeId.Equals(id));
                    
                    return ptEntry;
                }).ToList();

                gameToSave.Publisher = database.Publishers.Get(m => m.PublisherId.Equals(game.PublisherId));
